...
the options, the 106 addresses out of the symbol file, and the offer to show a
...
- **The second device does not need the `.sym` at all.** The file is 1.8MB and
  the app reads 106 symbols in it, so the room carries those 106 addresses —
...
| the `.sym` file | **never** | — |
| 106 addresses out of it | yes, ~1KB | while sharing |
