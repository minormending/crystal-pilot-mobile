...
the options, the 100 addresses out of the symbol file, and the offer to show a
...
- **The second device does not need the `.sym` at all.** The file is 1.8MB and
  the app reads 100 symbols in it, so the room carries those 100 addresses —
...
| the `.sym` file | **never** | — |
| 100 addresses out of it | yes, ~1KB | while sharing |
