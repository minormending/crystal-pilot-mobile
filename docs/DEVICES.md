...
the options, the 105 addresses out of the symbol file, and the offer to show a
...
- **The second device does not need the `.sym` at all.** The file is 1.8MB and
  the app reads 105 symbols in it, so the room carries those 105 addresses —
...
| the `.sym` file | **never** | — |
| 105 addresses out of it | yes, ~1KB | while sharing |
