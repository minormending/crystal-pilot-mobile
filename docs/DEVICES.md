...
the options, the 61 addresses out of the symbol file, and the offer to show a
...
- **The second device does not need the `.sym` at all.** The file is 1.8MB and
  the app reads 61 symbols in it, so the room carries those 61 addresses —
...
| the `.sym` file | **never** | — |
| 61 addresses out of it | yes, ~1KB | while sharing |
