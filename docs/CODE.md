...
8. [The errands](#8-the-errands)
   ...
   · [Reaching an hour](#8i-reaching-an-hour)
...
`loadState()` **restores the machine exactly, and may be called once.** That
sentence has been wrong here in both directions, so it is worth setting out what
was measured rather than what was concluded.

Take a state, step 1,500 frames with no input at all, and restore: 1,315 bytes
of work RAM moved, and **0 differ** afterwards. Not "mostly", not "all but the
stack" — zero, read immediately with no extra frame to muddy it. The game keeps
running, and stepping frames moves it on from where it was put back.

The second one always fails. Called again in the same page session it throws —
`Failed to convert value to 'object'`, or `undefined`, depending on where the
state came from — and a state fetched back from the library's own
`getSaveStates()` throws the same way on the first attempt. Re-priming with
`play()` before each save fixes the *save* half (without it the second
`saveState()` comes back with all four buffers empty, because the first load
transferred them to the worker) and does not help the load half at all.

So the ceiling is one restore per page, which is not something slots, undo or a
rewind can be built on: every one of those needs to go back more than once.
...
<!-- covers: gen2/symbols.js @ a35ba6450500 -->
...
[reaching an hour](#8i-reaching-an-hour). The hours of a playtime are two
...
<!-- covers: gen2/romdata.js @ 9486b83e460a -->
...
**And *what an object is* can be more than one number.** Crystal has three
object kinds and a trainer is 2. Polished Crystal has eight, of which two are
trainers — `OBJECTTYPE_TRAINER` and `OBJECTTYPE_GENERICTRAINER` — and **461 of
its 525 fightable objects are the second one**. Held to a single number this
saw an eighth of the trainers in the game, and every trainer in every Gym but
the leader is generic, so a "cleared" Gym would have been a Gym nobody fought.

So `objectTypes.trainer` is a number *or a list of them*, the same bargain
`encounter.blockOf` makes, and `kindsOf(engine, 'trainer')` in
`gen2/engine.js` is what the three callers ask instead of comparing. An
undeclared kind is an empty set, which is the generic profile's position:
...
clock *"is the real one, so this is advice about your evening rather than
something the pilot can hurry along."* **That was written down as a fact and
never measured, and it is wrong.**

### How Gen 2 actually tells the time

`FixTime` at 00:061d, read out of the cartridge:

```
    a = wStartSecond + rtcSeconds ; wrap at 60 ; -> hSeconds
    a = wStartMinute + rtcMinutes + carry ; wrap at 60 ; -> hMinutes
    a = wStartHour   + rtcHours   + carry ; wrap at 24 ; -> hHours
    a = wStartDay    + rtcDayLo   + carry             ; -> wCurDay
```

It **adds**. The four `wStart` bytes are an offset the game applies to the
cartridge's own real-time clock — which is how a Game Boy game lets you set the
time on hardware whose clock it cannot write. And all four are inside the saved
block, so **the in-game clock is editable in a `.sav`**.

The game does exactly that itself. `DSTChecks.SetClockForward` at 05:64b9 moves
the clock an hour by incrementing `wStartHour`, wrapping at 24 and carrying one
into `wStartDay` — which is the arithmetic `state.advanceClock` copies rather
than invents. The label names are the trap: *forward* increments the offset,
which only reads as forward because the offset is added.

`TimesOfDay` at 05:4044 is the other half, and `romdata.hourBlocks()` reads it.
It is a table of **upper bounds**, walked until one is greater than the hour —
so reading it as starts shifts every boundary by a block and still produces a
table. On this cartridge:

| block | hours | |
| --- | --- | --- |
| morning | 04:00–09:00 | 6 |
| day | 10:00–17:00 | 8 |
| night | 18:00–03:00 | 10, and it wraps midnight |
...
```mermaid
flowchart LR
    N["it is day · HOOTHOOT is after dark"] --> W["<b>Wait</b><br/>run the game to the hour"]
    N --> S["<b>Skip</b><br/>move the game's clock to it"]
    W --> WC["costs: minutes of frames<br/>keeps: everything"]
    S --> SC["costs: a ROM reload, so everything<br/>since the last in-game save<br/>keeps: the clock is instant"]
    WC --> WD{"does this core's RTC<br/>follow the frames?"}
    WD -- yes --> OK1["the hour arrives"]
    WD -- no --> OK2["it says so, in those words"]
...
— and then reads `wTimeOfDay` back, because the whole point of going through
the game's own clock is that the game decides what the hour means.

<details>
<summary><b>Advanced detail:</b> the checksum, the one-press shift, and what is
still not verified</summary>

**The battery is re-sealed or it is not written.** `state.checksum` is a 16-bit
sum over `sGameData` to `sGameDataEnd`, stored little-endian at `sChecksum` —
measured on a real cartridge, where the sum came to 58439 and the stored bytes
held 58439. Every one of those three addresses comes out of the symbol file.
`shiftClock` refuses a battery whose checksum *already* disagrees with its
bytes, because re-sealing that would turn a save the game refuses into a save
the game accepts and is wrong about.

**The shift is one press, and it can land short on purpose.** The app knows
which *block* it is in and not which hour — the live hour is `hHours` in HRAM
at `$ff94`, outside the work-RAM window every read here goes through, and the
core exposes `WORK_RAM_LOCATION` and nothing that would locate it. So
`shiftTo` lines the *earliest* hour this block could be onto the first hour of
the target. That is exactly right whenever the target is at least as wide as
where you are standing, and short by one block when it is not — night is ten
hours and morning is six. Rather than loop over that invisibly, the shift
happens, the block is read back, and the row offers the remainder. Two presses
at worst, and each is a whole answer.

**What is still not verified**, and it is the same gap the save editor was
parked on: an *edited* save has never been watched installing and reading back
changed. `install` refuses on a hidden page — loading a save restarts the
emulator, which a hidden tab cannot do — and the sessions this was built in
did not have a visible one. The machinery under it is not new, though: writing
a battery and driving CONTINUE is how loading a slot has always worked. What is
new is that the bytes were changed on the way, and the checksum is what decides
whether the game accepts them.
...
**Its wild levels are not levels.** `LEVEL_FROM_BADGES` is 178, and a slot
writes `LEVEL_FROM_BADGES + 1` for one above whatever `wBadgeBaseLevel`
currently is; `AdjustLevelForBadges` subtracts the constant, adds the base and
clamps to 2..99. **357 of its 3171 filled slots are written that way** — Route
47, Route 48, most of Kanto — so read literally the app offers a Lv179 Ditto
and the Hunt row writes off a patch of grass the pilot could clear.

The base is in work RAM, so `wildLevels` takes the snapshot and a caller
without one gets **null**, which every reader already treats as "this grass
says nothing about levels". Null rather than the sentinel, because 179 is not
wrong by a little. Crystal's `levelFromBadges` is null and its levels are
levels.

Found by `tools/dex --wilds`, which was written to check the *stride* — and
the stride was right. 357 sentinels came back as levels between 172 and 183 in
a walk that was looking for something else entirely.
...
<!-- covers: app/main.js index.html @ ef6df0e20d2c -->
...
Where the picture comes from is in `romdata.js` — `speciesIcon`, and two facts
that are not guessable and look like working code when wrong. Tiles are
**row-major** within a frame rather than the column-major order a Game Boy 8×16
sprite uses; and `PokemonPalettes` is indexed by **`species`, not
`species - 1`**, because it has a leading entry. With the usual minus-one
Pikachu decodes purple — wrong, but not visibly wrong, which is the kind of
defect that survives a review.

**There are two icon tables in the wild and neither reader can read the
other's**, so `iconKind` in the engine profile says which rather than anything
sniffing the data. Crystal indirects through a family: a byte per species
naming one of 37 outlines, a pointer per outline, one bank of uncompressed
tiles. It maps 251 species onto those 37, the most-used covering thirty of them
and only six species having one to themselves, so what tells a Chikorita from a
Bellsprout there is the palette. Polished Crystal gives every species its own
row in one table — `MiniIconPointers`, seven bytes, a bank and three addresses —
compressed, with a transparency mask beside it. A cartridge whose symbol file
names neither answers `null` and the lens is a lens with nothing in it.

**The mask is the one thing about the second shape a renderer cannot infer.**
Those tiles use all four palette indices, so colour 0 is a real white *inside*
the silhouette and transparency *outside* it, and the tile data does not
separate the two. The mask is one bit per pixel over the same eight tiles and
says which. Without it a Charmander loses its belly and a Snorlax is mostly
hole — which is exactly what the front pic, having no mask, does look like.
`_miniIcon` therefore returns pixels one index higher than the tile data: 0
transparent, then white, light, dark, black, with `colours` saying so, because
that is all `paintSprite` reads.

**The cartridge's own colours were decoded and then declined**, and that is a
choice rather than an omission. `OverworldMonIconColors` holds two bytes a
species — normal and shiny — each a nibble pair indexing `MapObjectPals`, 28
palettes across 4 times of day. Decoding all 291 shows every one of them naming
light 13, whose colour is the same warm tan for every species: the game draws
all of them with one body colour and one species accent. That is Crystal's
family problem again, in colour instead of outline, and there is a better answer
already in the file — `PokemonPalettes`, per species, the two colours the dex
portrait uses. Taking the cartridge's would have made the lens and the Dex
disagree about what colour a Pokémon is.

**And `family` in the answer is why the lens now prefers an icon to a
portrait.** The order used to run portrait-first, on the grounds that an icon
says *something small and green* where a portrait says which — true of Crystal's
table, written down as though true of icons. On a cartridge drawing one per
species the same rule picked the worse picture: a battle-sized portrait shrunk
into a 40px circle, over artwork drawn for exactly that size, animated, at a
whole-number scale. So the order asks whether *this* icon identifies the
species, and Crystal's answer is unchanged.

**And the dex entry draws the front pic, which is per species.** A family shape
is fine for a thing you glance at and wrong for the pane you opened deliberately
to look something up, so `speciesPic` decompresses the real picture — 40×40 to
56×56, out of `PokemonPicPointers`, through Gen 2's own LZ.

Three things in that path are worth writing down, because each of them decodes
*something* when it is wrong:

* **The bank byte is offset.** `PokemonPicPointers` stores `BANK(pic) - $36`,
  because the pics live above bank `$36` and the assembler's macro takes the
  difference so it fits a byte. Nothing in the ROM says what the offset is, so
  it is a cartridge fact and lives in the engine profile. It was measured, not
  looked up: the table's first entry reads bank `$1d`, and `BulbasaurFrontpic`
  is at `$53`.
* **The LZ back-reference reads two ways.** A high bit on the first offset byte
  means *this many bytes back from here*; without it the two bytes are an
...
* **Pic tiles are column-major**, the opposite of the menu icon's row-major
  ...
  5×5, 6×6 or 7×7 tiles and the game centres them in a 7×7 box, so the drawn
  size follows the picture: a 40px Pikachu stretched to 56 is a fat Pikachu.
...
**And the picture moves, because the cartridge has always said how.** Gen 2
does not store whole frames. The decompressed stream is the base tiles followed
by a pool of spare ones — which is why a 5×5 Bulbasaur decompresses to 54 tiles
rather than 25 — and a frame is a *bitmask* naming which tile positions change
plus one tile index per set bit. Bulbasaur's first bitmask has thirteen bits set
and its first frame is fourteen bytes: the mask index and thirteen tiles. That
arithmetic is how the format was confirmed before a single pixel was drawn.

`speciesAnimation` assembles them and hands back every frame already painted,
plus the order to play them. Three things about it are worth keeping:

* **The script says how many frames there are; the table does not.** The first
  draft took the count from where the first pointer lands, on the reasoning
  that a list of `dw` ends where its own data begins. True of most species and
  false of some — Totodile's and Chikorita's first pointers go *backwards*, to
  frames defined ahead of the list, and the subtraction yields a negative
  count. Nothing needs the total; what is needed is the frames the script names.
* **The loops are flattened rather than run.** `setrepeat` and `dorepeat` are
  read for their arguments and skipped, so what comes back is every frame the
  script names, once, in order, and the lens cycles that. It is not the
  cartridge's exact timing and does not need to be — a loop interpreter is a
  second thing to get wrong for a benefit nobody can see.
* **Where the frames are reached differs.** Crystal has a `FramesPointers`
  table; Polished keeps the address in the picture's own row instead, which is
  what the seventh byte of its entry is. Both are profile facts.

Coverage, measured: 250 of Crystal's 251 animate — only Unown does not, and it
falls back to the menu icon — and all 291 of Polished's do, though there the
...
- **And so is how wide one pointer is**, which is the same property one level
  down. Crystal writes `dw` — two bytes — and pokecrystal16 writes `dba`: a
  bank byte and then the address, because its trainer data outgrew one bank.
  Read at the wrong width the table still decodes, into **503 classes instead
  of 67** with names like `"ser? ATTACK."`. So each candidate width is tried
  and one is kept: the first entry's address must be past the table, the gap
  must divide by the width, and it must land in the same 16KB window as the
  table. Only one width passes on each cartridge, and both then agree the
...
rule against **every map the app can read** — 361 on Crystal: 1336 objects,
twelve disagreements, every one of them explicable, because some trainers are
talked to rather than seen, so `TrainerOfficerDirk` is legitimately a script
object and the Trainer House's receptionist is named `Trainer…` while being
nobody to fight.
...
Three constants were hand-copied and three were wrong, so `check-app
romlayout` read all seven out of both files and compared them — the repair for
*two copies of a structure* rather than for carelessness. It was not enough.
Polished Crystal's coord event really is five bytes; both files said eight;
the check saw them agree. **The tool has no copy at all now** and asks
`tools/route`, and `romlayout` asks the cartridge — see [a cartridge that
changed everything it could](#8j-a-cartridge-that-changed-everything-it-could).

The low-nibble rule is Crystal's too, incidentally: Polished gives the palette
its own byte and writes the type whole, and it has two trainer kinds rather
than one. On that cartridge the same check reads 2108 objects with 22
disagreements.
...
<!-- covers: app/rows.js app/main.js index.html @ be1e93d31910 -->
...
<!-- covers: index.html app/main.js @ ef6df0e20d2c -->
...
all three merge: the remembered options, the 110 addresses out of the symbol
...
The `.sym` is 1.8MB and this app looks up **110 symbols in it**. So the room
carries those 110 lines — about a kilobyte, `{name: [bank, addr]}` — and a
...
and it reports 110 because that is how many symbols it has.

```mermaid
flowchart LR
    F["the .sym file<br/>1.8MB, 58,456 symbols"] --> S["Symbols<br/>the parsed table"]
    S -->|"digest(SHARED_SYMBOLS)"| D["{name: [bank, addr]}<br/>110 entries, ~1KB"]
    D --> R[["the room"]]
