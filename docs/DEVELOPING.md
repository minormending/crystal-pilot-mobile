...
686 tests in 26 files, and what each file is about says more than the count:
...
| `battle.mjs` | 52 | whose turn it is, which Pokémon is out, and a win from a whiteout |
...
| `romdata.mjs` | 25 | the cartridge's own character encoding and tables, byte by byte |
